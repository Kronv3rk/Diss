...
def build_series(runs: list) -> dict:
    ...
    for algo in algos_present:
        ...
        for m in METRICS:
            ...
            _lo, _hi = bootstrap_ci_mean(vals, seed=42)
            agg[f"{m}_boot_lo"] = _lo
            agg[f"{m}_boot_hi"] = _hi
        agg["n_runs"] = n
        ...
        for algo in algos_present:
            ...
            for m in ["D_mean", "M_cum"]:
                ...
                if valid.sum() >= 4:
                    ...
                    row[f"p_{m}"] = None
                _d, _mag = cliffs_delta(b_vals[valid], a_vals[valid])
                row[f"cliffs_{m}"] = _d
                row[f"cliffs_{m}_mag"] = _mag
