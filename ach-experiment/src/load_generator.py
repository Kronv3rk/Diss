...
class LoadGenerator:
    ...
    def __init__(self, NK: int = 50_000, zipf_s: float = 0.0, seed: int = 42,
        ...
        # Popularity: explicit real trace > Zipf > uniform
        if key_probs is not None:
            kp = np.asarray(key_probs, dtype=np.float64)
            if kp.shape[0] != self.NK:
                raise ValueError(f"key_probs length {kp.shape[0]} != NK {self.NK}")
            self._key_probs = kp / kp.sum()
        elif zipf_s > 0.0:
