...
from .algorithms.ch_bounded import CHBoundedLoads
from .algorithms.dynamic_r import DynamicR
from .algorithms.rendezvous import HRW
...
from .metrics import aggregate_metrics, compute_D
...
def _build_algorithms(cfg: dict, ring_snapshot: HashRing) -> list:
    ...
    algos = [
        ...
        CHBoundedLoads(epsilon=0.25, v_min=int(ach_params.get("v_min", 3))),
        ACH(ach_params),
    ]
    return algos
...
def run_experiment(cfg: dict, seed: int = 42) -> dict:
    ...
    trace_path = cfg.get("trace")
    _key_probs = None
    if trace_path:
        from .trace_loader import load_popularity
        _tp = (trace_path if os.path.isabs(trace_path)
               else os.path.join(os.path.dirname(__file__), "..", trace_path))
        _key_probs = load_popularity(_tp, NK)
    gen = LoadGenerator(NK=NK, zipf_s=zipf_s, seed=seed, key_probs=_key_probs)

    _sigma_noise = float(noise_cfg.get("sigma", 0.0))
    _p_miss      = float(noise_cfg.get("p_miss", 0.0))
    _lag         = int(noise_cfg.get("lag", 0))

    algos = _build_algorithms(cfg, HashRing(V=V, n_nodes=n_nodes, seed=seed))
    n_algos = len(algos)
    ...
    rings = [HashRing(V=V, n_nodes=n_nodes, seed=seed) for _ in range(n_algos)]
    telemetries = [Telemetry(n_nodes, tel_params) for _ in range(n_algos)]
